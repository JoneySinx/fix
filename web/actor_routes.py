...
async function loadActorSection() {
    // sidebar बंद करो पहले (mobile पर)
...
# ─────────────────────────────────────────────────────────
# 📸 ACTOR THUMBNAIL — Dedicated Endpoint (actors collection)
# ─────────────────────────────────────────────────────────
@actor_routes.get("/api/actor_thumb")
async def api_actor_thumb(req):
    """Actors collection के लिए dedicated thumbnail — /api/thumb की तरह काम करता है"""
    actor_id = req.query.get("actor_id", "").strip()
    if not actor_id:
    ...
    try:
        ...
        thumb_val = actor_doc.get("thumb_url", "")
        if not thumb_val or not thumb_val.startswith("TG_ID:"):
            return web.Response(status=404)
        tg_file_id = thumb_val.replace("TG_ID:", "")
        ...
        logger.error(f"actor_thumb error: {e}")
...
async def api_actors_list(req):
    ...
    # ✅ FIX: cursor reassign (Motor chaining bug)
    cursor = actors.find({}, {"name": 1, "_id": 1}).sort("name", 1)
    actor_docs = await cursor.to_list(length=300)
    
    # ✅ FIX: /api/actor_thumb use करो — actors collection में खोजता है
...
async def api_get_actor_node(req):
    ...
    # ✅ FIX: gallery TG_ID: strip करके correct file_id निकालो
    gallery_links = []
    for img in actor.get("gallery", []):
        gid = img.replace("TG_ID:", "") if img.startswith("TG_ID:") else img
        gallery_links.append(f"/api/thumb?file_id={gid}&col=primary")
    ...
    return web.json_response({
        "actor": {
            ...
            # ✅ FIX: dedicated actor_thumb endpoint
