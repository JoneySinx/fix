    ...
    try{
        ...
        d.results.forEach(f=>{
            ...
            if(pMode!=='none'){
                ...
                imgHtml=`<img src="${imgSrc}" class="fc-poster" onerror="this.src='https://i.ibb.co/30B3RcS/default-movie.png'">`;
            }
            
            h+=`<div class="file-card">${imgHtml}<div class="fc-left"><div class="fc-top"><span class="source-badge ${sc}">${sc.toUpperCase()}</span><span class="type-tag">${f.type.toUpperCase()}</span></div><div class="fc-name">${f.name}</div><div class="fc-meta">Size: ${f.size}</div>${adminControls}</div><a href="${f.watch}" target="_blank" class="btn-play">&#9654; Play</a></div>`;
        });
...
async def dash(req):
    ...
    # ✅ UI FIX: यहाँ Poster Mode सेलेक्ट करने का ड्रॉपडाउन लगा दिया गया है
