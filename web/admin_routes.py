...
# ✅ FIX: JS Update - Added handleThumbError & reloadThumb for Web Parity
JS = """
...
async function reloadThumb(fileId) {
    ...
    testImg.src = '/api/thumb?file_id=' + fileId + '&retry=true&t=' + timestamp;

    testImg.onload = function() {
        var box = document.getElementById('poster-box-' + fileId);
        if (box) { box.innerHTML = '<img src="' + testImg.src + '" class="fc-poster" onerror="handleThumbError(\\''+fileId+'\\')">'; }
    };
    ...
    testImg.onerror = function() {
        if (btn) {
            ...
            showToast("टेलीग्राम अभी बिजी है, 5 सेकंड बाद फिर दबाएं।", "error");
        }
    };
}
...
async function doSearch(o){
    ...
    try{
        ...
        d.results.forEach(f=>{
            ...
            var imgHtml='';
            if(pMode!=='none'){
                imgHtml=`<div class="poster-box" id="poster-box-${f.file_id}"><img src="${f.poster}" class="fc-poster" onerror="handleThumbError('${f.file_id}')" loading="lazy"></div>`;
            }
