...
# ✅ FIX: JS में res.message की जगह सही toast messages
JS = """
...
async function doSearch(o){
    ...
    try{
        ...
        d.results.forEach(f=>{
            ...
            if(pMode!=='none'){
                var imgSrc=f.poster;
                if(pMode==='tg') imgSrc=f.tg_thumb || `/api/thumb?file_id=${f.file_id}`;
...
// ✅ FIX: res.message → सही toast message (backend में message key नहीं होती)
...
async function editFile(fid,col,oldName){
    ...
    try{
        ...
        var res=await r.json();
        if(res.success){showToast('✅ File renamed successfully!');doSearch(curOff);}
        else{showToast(res.error||'Edit failed!','error');}
...
# ✅ FIX: Python Syntax Fix for Quotes
