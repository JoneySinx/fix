...
async def stats(req):
    ...
    # Absolute Plain String formatting (No f modifier) to prevent brackets mismatch error
    # Variables are safely embedded through manual string casting blocks
    total_val = f"{s.get('total', 0):,}"
    primary_val = f"{s.get('primary', 0):,}"
    primary_thumb = f"{s.get('primary_thumb', 0):,}"
    cloud_val = f"{s.get('cloud', 0):,}"
    cloud_thumb = f"{s.get('cloud_thumb', 0):,}"
    archive_val = f"{s.get('archive', 0):,}"
    archive_thumb = f"{s.get('archive_thumb', 0):,}"
    total_thumb = f"{s.get('total_thumb', 0):,}"
    bot_users = f"{u:,}"

    html_stats_body = '<div class="main" style="padding-top:40px;">' \
        '<div class="big-stat">' \
        ...
        '<div class="stats-row">' \
            '<div class="scard red">' \
                ...
                '<div class="scard-val">' + primary_val + '</div>' \
                '<div class="scard-sub">🖼️ Verified Thumbs: <b>' + primary_thumb + '</b></div>' \
            '</div>' \
            '<div class="scard white">' \
                '<div class="scard-label">Series (Cloud)</div>' \
                '<div class="scard-val">' + cloud_val + '</div>' \
                '<div class="scard-sub">🖼️ Verified Thumbs: <b>' + cloud_thumb + '</b></div>' \
            '</div>' \
            '<div class="scard grey">' \
                '<div class="scard-label">Archive (Backup)</div>' \
                '<div class="scard-val">' + archive_val + '</div>' \
                '<div class="scard-sub">🖼️ Verified Thumbs: <b>' + archive_thumb + '</b></div>' \
            '</div>' \
            '<div class="scard red">' \
                '<div class="scard-label">Global Cached Thumbs</div>' \
                '<div class="scard-val">' + total_thumb + '</div>' \
