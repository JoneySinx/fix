...
/* ══════════════════════════════════════════
   EDIT MODAL — inline styles (no extra CSS needed)
══════════════════════════════════════════ */
function _closeModal(){var m=document.getElementById('editModal');if(m)m.remove();}

function editFile(fid, col, oldName){
    _closeModal();
    ...
    overlay.innerHTML=`
      <div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:28px;width:100%;max-width:420px;position:relative;">
        <button onclick="_closeModal()" style="position:absolute;top:12px;right:14px;background:0 0;border:0;color:var(--muted);font-size:20px;cursor:pointer;">&#10005;</button>
        <h3 style="margin-bottom:20px;font-size:18px;font-weight:900;">&#9999;&#65039; Edit File</h3>

        <!-- File Name -->
        <div style="margin-bottom:16px;">
          <label style="font-size:11px;font-weight:700;color:var(--muted);display:block;margin-bottom:6px;">FILE NAME</label>
          <input id="em_name" type="text" value="${oldName.replace(/"/g,'&quot;')}"
            style="width:100%;background:var(--bg);border:1px solid var(--border);border-radius:6px;padding:11px 13px;color:var(--text);font-size:14px;outline:0;"
            onfocus="this.style.borderColor='var(--accent)'" onblur="this.style.borderColor='var(--border)'">
        </div>

        <!-- Thumbnail Upload -->
        <div style="margin-bottom:20px;">
          <label style="font-size:11px;font-weight:700;color:var(--muted);display:block;margin-bottom:8px;">THUMBNAIL</label>
          <div id="em_preview" style="width:100%;aspect-ratio:16/9;border-radius:6px;background:var(--bg3);border:2px dashed var(--border);overflow:hidden;position:relative;display:flex;align-items:center;justify-content:center;margin-bottom:10px;cursor:pointer;" onclick="document.getElementById('em_file').click()">
            <img id="em_prev_img" src="/api/thumb?file_id=${fid}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;display:none;" onload="this.style.display='block';document.getElementById('em_prev_placeholder').style.display='none';" onerror="this.style.display='none';">
            <div id="em_prev_placeholder" style="text-align:center;color:var(--muted);pointer-events:none;">
              <div style="font-size:28px;margin-bottom:6px;">&#128444;&#65039;</div>
              <div style="font-size:12px;font-weight:700;">Click to upload image</div>
              <div style="font-size:11px;margin-top:4px;">JPG / PNG / WEBP</div>
            </div>
          </div>
          <input id="em_file" type="file" accept="image/jpeg,image/png,image/webp" style="display:none;" onchange="em_previewLocal(this)">
          <button onclick="document.getElementById('em_file').click()" style="width:100%;background:var(--bg3);color:var(--text);border:1px solid var(--border);border-radius:6px;padding:10px;font-size:13px;font-weight:700;cursor:pointer;">&#128193; Choose Image</button>
        </div>

        <button id="em_save_btn" onclick="em_save('${fid}','${col}')" style="width:100%;background:var(--accent);color:#fff;border:0;border-radius:6px;padding:14px;font-size:15px;font-weight:900;cursor:pointer;">&#128190; Save Changes</button>
      </div>`;
    document.body.appendChild(overlay);
    overlay.addEventListener('click',function(e){if(e.target===overlay)_closeModal();});
}

function em_previewLocal(input){
    if(!input.files||!input.files[0])return;
    var reader=new FileReader();
    reader.onload=function(e){
        var img=document.getElementById('em_prev_img');
        var ph=document.getElementById('em_prev_placeholder');
...
async function em_save(fid, col){
    var nameVal=(document.getElementById('em_name').value||'').trim();
    var fileInput=document.getElementById('em_file');
    var hasImage=fileInput&&fileInput.files&&fileInput.files.length>0;
    var btn=document.getElementById('em_save_btn');
    if(!nameVal&&!hasImage){showToast('Kuch toh badlo — naam ya thumbnail!','error');return;}
    ...
    try{
        if(hasImage){
            ...
            var tr=await fetch('/api/upload_thumb',{method:'POST',body:fd});
            var tj=await tr.json();
            if(!tj.success){showToast(tj.error||'Thumb upload fail!','error');btn.disabled=false;btn.textContent='&#128190; Save Changes';return;}
            var cardBox=document.getElementById('poster-box-'+fid);
            if(cardBox&&tj.preview){cardBox.innerHTML='<img src="'+tj.preview+'" class="fc-poster">';}
        }
        if(nameVal){
            var r=await fetch('/api/edit',{method:'POST',body:JSON.stringify({file_id:fid,collection:col,new_name:nameVal}),headers:{'Content-Type':'application/json'}});
            var res=await r.json();
            if(!res.success){showToast(res.error||'Naam update fail!','error');btn.disabled=false;btn.textContent='&#128190; Save Changes';return;}
        }
        showToast('&#10003; File successfully update ho gaya!');
        _closeModal();
        doSearch(curOff);
    }catch(e){
