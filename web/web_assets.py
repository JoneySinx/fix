...
def build_page(title, body, cls="", active_tab="", role=None):
    # 🎭 ✅ फिक्स 1: एक्टर्स बटन पर क्लिक करने पर साइडबार छुपाओ और जावास्क्रिप्ट को ट्रिगर करो
    actor_click = "closeSidebar(); loadActorSection();"
    
    # 🎭 ✅ फिक्स 2: साइडबार मेनू के अंदर "Home" के ठीक नीचे बटन को आपके पसंदीदा डिज़ाइन में इंजेक्ट किया गया
    if role == 'admin': 
        nav_links = f'<a href="/dashboard" class="sb-link {"active" if active_tab=="dash" else ""}">Home</a><button class="sb-link" onclick="{actor_click}">🎭 Actors</button><a href="/stats" class="sb-link {"active" if active_tab=="stats" else ""}">Database Stats</a><a href="/profile" class="sb-link {"active" if active_tab=="profile" else ""}">Profile Settings</a>'
    elif role == 'user': 
        nav_links = f'<a href="/dashboard" class="sb-link {"active" if active_tab=="dash" else ""}">Home</a><button class="sb-link" onclick="{actor_click}">🎭 Actors</button><a href="/profile" class="sb-link {"active" if active_tab=="profile" else ""}">Profile Settings</a>'
    else: 
        nav_links = ""
